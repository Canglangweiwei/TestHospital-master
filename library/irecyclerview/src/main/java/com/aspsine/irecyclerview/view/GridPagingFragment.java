...
/**
 * <p>
 * 可分页List的Fragment，使用时继承此类，重写GridPageable接口中的方法来绑定Adapter、每次请求数据以及重写加载数据方法。
...
public abstract class GridPagingFragment extends Fragment implements GridPageable {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (view == null) {
            view = inflater.inflate(R.layout.fragment_page_grid, container, false);
            initView();
        }
        return view;
    }
    ...
    public GridLayoutManager getGridLayoutManager() {
        return new GridLayoutManager(getActivity(), defaultPartOnlineNumber());
    }

    @Override
    public StaggeredGridLayoutManager getStaggeredGridLayoutManager() {
        return new StaggeredGridLayoutManager(defaultPartOnlineNumber(), StaggeredGridLayoutManager.VERTICAL);
    }
...
//        }
        ...
        StaggeredGridLayoutManager gridLayoutManager = getStaggeredGridLayoutManager();
        if (gridLayoutManager != null) {
            recyclerView.setLayoutManager(gridLayoutManager);
        }
    }
